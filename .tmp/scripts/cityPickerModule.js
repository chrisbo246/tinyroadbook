...
var cityPickerModule = function () {
    ...
    var settings = {
        quill: {
            ...
            //theme: 'snow',
            styles: false,
            formats: ['bold', 'italic', 'strike', 'underline', 'font', 'size', 'align', 'color', 'background'] //'bullet', 'list', 'image', 'link'
        },
        spinner: {
            color: '#fff',
            scale: 0.75
    ...
    var _insertCity = function (json) {
        ...
        var size,
            color,
            names = [],
            types = ['country', 'state', 'county', 'city', 'town', 'village', 'hamlet']; //'state_district', 'city_district',
    ...
    var _getCity = function (position, zoom) {
        ...
        geocodeModule.nominatimReverse(params).success(function (json) {
            ...
            setTimeout(function () {
                return _getCityDetails(json).success(function (json) {
                    console.log(JSON.stringify(json));
                    _insertCity(json[0]);
                }).done(function () {
                    spinner.stop();
    ...
    var _print = function (data) {
        ...
        printWin.document.write('<html>' + '<head><title>' + window.location.hostname + '</title></head>' + '<body>' + data + '</body></html>');
        printWin.print();
        printWin.close();
